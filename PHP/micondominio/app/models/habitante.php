<?php

class Habitante extends MvcModel {
    var $table = '{prefix}xmc_HBTNT';    
    var $primary_key = 'id';
    var $selects = ['id', 'user_id', 'condominio_id', 'nombre', 'direccion', 'estado_id', 'saldo_pagar', 'fecha_saldo'];
    var $validate = [
        'user_id' => [
            'message' => 'user_id no v&aacute;lido',
            'required' => true,
            'pattern' => '/^(\+|-)?\d+$/'
        ],
        'condominio_id' => [
            'message' => 'condominio_id no v&aacute;lido',
            'required' => true,
            'pattern' => '/^(\+|-)?\d+$/'
        ],
        'nombre' => [
            'message' => 'nombre no v&aacute;lido',
            'required' => true,
            'rule' => 'alphanumeric'
        ],
        'direccion' => [
            'message' => 'direccioninterna no v&aacute;lido',
            'required' => true,
            'rule' => 'alphanumeric'
        ],
        'estado_id' => [
            'message' => 'estado_id no v&aacute;lido',
            'required' => true,
            'pattern' => '/^(\+|-)?\d+$/'
        ],
        'saldo_pagar' => [
            'message' => 'saldoPorPagar no v&aacute;lido',
            'required' => true,
            'rule' => 'numeric'
        ],
        'fecha_saldo' => [
            'message' => 'fechasaldo no v&aacute;lido',
            'required' => true,
            'pattern' => '/^(?:[\+-]?\d{4}(?!\d{2}\b))(?:(-?)(?:(?:0[1-9]|1[0-2])(?:\1(?:[12]\d|0[1-9]|3[01]))?|W(?:[0-4]\d|5[0-2])(?:-?[1-7])?|(?:00[1-9]|0[1-9]\d|[12]\d{2}|3(?:[0-5]\d|6[1-6])))(?:[T\s](?:(?:(?:[01]\d|2[0-3])(?:(:?)[0-5]\d)?|24\:?00)(?:[\.,]\d+(?!:))?)?(?:\2[0-5]\d(?:[\.,]\d+)?)?(?:[zZ]|(?:[\+-])(?:[01]\d|2[0-3]):?(?:[0-5]\d)?)?)?)?$/'
        ]
    ];
    var $belongs_to = [
        'Condominio' => [ 'foreign_key' => 'condominio_id' ]
    ];
    var $per_page = 7;
    var $display_field = 'nombre';


}
