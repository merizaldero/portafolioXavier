(function($) {
    ...
    function xtt_bind_canvas_events() {
        $( '#xtt-drawing-area' ).on( 'click', function( e ) {
            ...
            switch ( xtt_current_tool ) {
                ...
                case 'text':
                    var text_content = prompt( xtt_localize_text || 'Ingrese texto:', '' );
                    if ( text_content ) {
        ...
        $( '#xtt-svg-content' ).closest( 'form' ).on( 'submit', function() {
            xtt_save_svg();
        } );

        $( '#xtt-drawing-area' ).on( 'dblclick', function( e ) {
            if ( xtt_current_tool === 'path' ) {
                return;
            }
            ...
            xtt_drawing_element = xtt_canvas.polygon( '' )
                .fill( fill_color )
                .stroke({ color: stroke_color, width: stroke_width })
                .opacity( opacity );

            xtt_drawing_element.on( 'click', function() {
                xtt_drawing_element = null;
            } );
        } );
    }

    function xtt_load_existing_svg() {
        var existing_svg = $( '#xtt-svg-content' ).val();
    ...
    function xtt_clear_canvas() {
        if ( confirm( xtt_localize_clear || 'Está seguro de limpiar todo el canvas?' ) ) {
